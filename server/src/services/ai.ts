...
async function analyzeNoteSingle(
  ...
  const systemPrompt = `你是一位笔记内容提取助手。你的唯一任务是从用户提供的笔记原文中，逐条提取笔记中已经明确写出的内容。你必须严格遵守以下铁律：
...
8. 题目识别：如果笔记内容是题目/试卷格式（包含题干、A/B/C/D选项、答案），请把每道题目作为一个知识点提取，name为题干，description包含完整选项、正确答案和解析（如果原文有）。
...
3. knowledgePoints: 严格依据笔记原文提取所有知识点。笔记中写了什么，你就提取什么；笔记没写的，你一句都不许编。每个知识点必须包含：
   - name: 知识点名称或题目题干，优先使用笔记原文中的叫法，不超过15个字。
   - description: 详细解释（80-400字），内容必须完全来自笔记原文的具体描述，可以精简整理语句，但**绝不允许加入原文没有的信息**。如果笔记是题目格式，description中必须保留完整的选项和正确答案。
   - domain: 所属学科领域。
...
7. 题目识别：如果片段内容是题目/试卷格式（包含题干、A/B/C/D选项、答案），请把每道题目作为一个知识点提取，name为题干，description包含完整选项、正确答案和解析（如果原文有）。
...
3. knowledgePoints: 严格依据片段原文提取所有知识点。片段中写了什么，你就提取什么；片段没写的，你一句都不许编。每个知识点必须包含：
   - name: 知识点名称或题目题干，优先使用片段原文中的叫法，不超过15个字。
   - description: 详细解释（80-400字），内容必须完全来自片段原文的具体描述，可以精简整理语句，但**绝不允许加入原文没有的信息**。如果片段是题目格式，description中必须保留完整的选项和正确答案。
   - domain: 所属学科领域。
...
【最高优先级规则】考察方向铁律：
1. 每道题必须直接考察知识点的"自身属性"：定义、含义、翻译、词性、核心特征、分类、辨析、公式结构、变量含义。
2. 绝对禁止考察该知识点所指代的外部事物或对象。知识点是什么，你就考察什么本身。
3. 如果知识点是"单词/短语/翻译类"，必须考察其含义、翻译、词性、拼写、用法搭配。
4. 如果知识点是"术语/概念/定义类"，必须考察其内涵、定义、性质、适用条件、核心特征。
5. 如果知识点是"公式/定理类"，必须考察其表达式、适用条件、推导逻辑、变量含义、单位。
6. 如果设置场景，场景只能是"知识点在语言/学科中的应用"场景，绝对不能变成"知识点所指代事物的实际场景"。
...
4. 如果知识点是"术语/概念/定义类"，必须考察其内涵、定义、性质、适用条件、核心特征。
5. 如果知识点是"公式/定理类"，必须考察其表达式、适用条件、推导逻辑、变量含义、单位。
6. 题目必须直接围绕知识点内容出题，禁止设置任何场景或案例，禁止出应用题。
