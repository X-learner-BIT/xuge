...
考察方向铁律（必须严格遵守，这是最重要的规则）：
1. 你必须考察知识点**本身**的属性：定义、含义、翻译、词性、核心特征、分类、与其他概念的辨析。
2. 绝对禁止考察该知识点所指代的外部事物或对象。例如：如果知识点是"apple（苹果）"，你必须考察"apple的含义/翻译/词性/用法"，绝对不能考察"苹果这种水果的产地、颜色、营养价值"。
3. 如果知识点是"术语/概念/定义"，考察其内涵、定义、性质、适用条件，不要考察该术语在实际世界中的具体例子或应用故事。
4. 如果知识点是"公式/定理"，考察其表达式、适用条件、推导逻辑、变量含义，不要考察用该公式计算的某个具体数值。
